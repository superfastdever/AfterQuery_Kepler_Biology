...
### 1. Domain scope — Life Sciences only

The owner's Kepler application was accepted for **Life Sciences**. Every task in
this repo maps to Kepler's **Science** category with the **Biology** label
($300 tier).

The general Kepler guidance never repeats this restriction, and it lists example
tasks across seven domains. That silence does **not** widen the scope, and those
examples are *format* references only. Do not author a task outside Life
Sciences.
